...
// ── Load inventory from admin panel (localStorage) ──────────
function getInventory() {
  try {
    const cars = JSON.parse(localStorage.getItem('pch_cars') || '[]');
    if (cars.length > 0) return cars;
  } catch(e) {}
  // Fallback demo cars if no admin listings yet
  return [
...
function renderInventory(list) {
  ...
  if (list.length === 0) {
    grid.innerHTML = '<p style="color:#888;text-align:center;padding:40px;">No cars available at this time. Check back soon!</p>';
    return;
  }
  ...
  list.forEach(car => {
    ...
    card.innerHTML = `
      <img src="${img}" alt="${car.make} ${car.model}" style="width:100%;height:200px;object-fit:cover;">
      <div class="card-body">
        ...
        ${car.desc ? `<p style="font-size:13px;color:#666;margin:6px 0;">${car.desc.substring(0,100)}${car.desc.length>100?'...':''}</p>` : ''}
        <div class="price">$${Number(car.price).toLocaleString()}</div>
        ${car.financeWeekly ? `<div style="font-size:13px;color:#888;">Finance from $${car.financeWeekly}/week</div>` : ''}
        <button class="btn btn-primary" onclick="addToCart(${car.id})" style="margin-top:10px;">Add to Cart</button>
...
// ── Update make filter dropdown with admin cars ──────────────
